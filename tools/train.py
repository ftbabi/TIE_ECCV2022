...
from sim.apis import set_random_seed, train_model
from sim.datasets import build_dataset
from sim.models import build_simulator
from sim.utils import collect_env, get_root_logger, count_parameters
...
def main():
    ...
    if args.launcher == 'none':
        ...
        if args.launcher != 'slurm':
            port = cfg.dist_params.pop('port', None)
            if port is not None:
    ...
    if cfg.checkpoint_config is not None:
        # save sim version, config file content and class names in
